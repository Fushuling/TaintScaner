...
class AstToIrConverter
{
    ...
    public function StmtParse($stmt)
    {
        ...
        if ($stmt->expr instanceof PhpParser\Node\Expr\Include_) {
            ...
            $this->quads[$this->quadId] = new Q_IR($this->quadId, $stmt->expr->getType(), null, $stmt->expr->expr, null);
        }
        ...
        if ($stmt instanceof PhpParser\Node\Stmt\Echo_) {
            ...
            for ($i = 0; $i < count($stmt->exprs); $i++) {
                if ($stmt->exprs[$i] instanceof PhpParser\Node\Expr\Variable) {
                    $this->quadId += 1;
                    $this->quads[$this->quadId] = new Q_IR($this->quadId, $stmt->getType(), null, $stmt->exprs[$i], null);
                }
            }
        }
    ...
    public function EncapsedParse($expr)
    {
        ...
        $this->quads[$this->quadId] = new Q_IR($this->quadId, "Whole_Encapsed_Start", null, null, null);
        for ($i = 0; $i < count($expr->parts); $i++) {
            if ($expr->parts[$i] instanceof PhpParser\Node\Expr\Variable) {
                $this->quadId += 1;
                $this->quads[$this->quadId] = new Q_IR($this->quadId, "Encapsed_Param", null, null, $expr->parts[$i]);
            }
        }
        if ($Whole_Encapsed_start_id == $this->quadId) {
            ...
            $this->quads[$this->quadId] = new Q_IR($this->quadId, "Whole_Encapsed_End", $Whole_Encapsed_start_id, $Whole_Encapsed_end_id, null);
        }
    }
    ...
    public function ExprParse($expr)
    {
        ...
        if ($expr instanceof PhpParser\Node\Expr\AssignOp\Concat) {
            ...
            if ($expr->expr instanceof PhpParser\Node\Scalar\String_) {
                ...
                $this->quadId += 1;
                $this->quads[$this->quadId] = new Q_IR($this->quadId, "Expr_Assign", $expr->expr, $expr->var, $expr->var);
                ...
                if ($before_id != $now_id) {
                    $this->quadId += 1;
                    $this->quads[$this->quadId] = new Q_IR($this->quadId, "Expr_Assign", null, $now_id, $expr->var);
                }
            }
        }
        ...
        if ($expr instanceof PhpParser\Node\Expr\Assign) {
            ...
            if ($expr->expr instanceof PhpParser\Node\Scalar) {
                ...
                if ($expr->expr instanceof PhpParser\Node\Scalar\Encapsed) {
                    ...
                    if ($before_id != $now_id) {
                        $this->quadId += 1;
                        $this->quads[$this->quadId] = new Q_IR($this->quadId, $expr->getType(), null, $now_id, $expr->var);
                    } else {
                        $this->quadId += 1;
                        $this->quads[$this->quadId] = new Q_IR($this->quadId, $expr->getType(), null, null, $expr->var);
                    }
                } else {
                ...
                $this->quadId += 1;
                //若为变量，则右操作数为当前QuadId
                $this->quads[$this->quadId] = new Q_IR($this->quadId, $expr->getType(), null, $expr->expr, $expr->var);
        ...
        if ($expr instanceof PhpParser\Node\Expr\BinaryOp\Concat) {
            ...
            if (
                ...
                $this->quads[$this->quadId] = new Q_IR($this->quadId, $expr->getType(), $expr->left, $expr->right, null);
            }
        }
        ...
        if ($expr instanceof PhpParser\Node\Expr\ArrayDimFetch) {
            ...
            $this->quads[$this->quadId] = new Q_IR($this->quadId, $expr->getType(), $expr->var, $expr->dim, null);
        }

        if ($expr instanceof PhpParser\Node\Expr\Exit_) {
            $this->quadId += 1;
            $this->quads[$this->quadId] = new Q_IR($this->quadId, $expr->getType(), null, null, null);
        }
            ...
            if ($expr->name instanceof PhpParser\Node\Identifier) {
                ...
                if (array_key_exists($expr->name->name, $SinkAll)) {
                    ...
                    for ($i = 0; $i < $param_count; $i++) {
                        $this->quadId += 1;
                        //调用的时候传入的参数
                        $this->quads[$this->quadId] = new Q_IR($this->quadId, "Internal_Call_Param", null, null, $expr->args[$i]);
                    }
                    $this->quadId += 1;
                    $id = $this->quadId;
                    $this->quads[$this->quadId] = new Q_IR($this->quadId, "Expr_FuncCall_Internal", $expr->name, null, null);
                    $this->quads[$id]->dest = $this->quadId + 1;
                    $end_id = $this->quadId;
                    $this->quadId += 1;
                    $this->quads[$this->quadId] = new Q_IR($this->quadId, "Expr_FuncCall_Internal_Finish", $start_id, $end_id, null);
                }
            }
        }
            ...
            if ($expr->name instanceof PhpParser\Node\Name) {
                ...
                if (in_array($expr->name->parts[0], get_defined_functions()["internal"])) { {
                        ...
                        if (!in_array($expr->name, $SanitizerAll)) {
                            ...
                            $this->quadId += 1;
                            $id = $this->quadId;
                            $this->quads[$this->quadId] = new Q_IR($this->quadId, $expr->getType() . "_Internal", $expr->name, null, null);
                            $this->quads[$id]->dest = $this->quadId + 1;
                            $end_id = $this->quadId;
                            $this->quadId += 1;
                            $this->quads[$this->quadId] = new Q_IR($this->quadId, "Expr_FuncCall_Internal_Finish", $start_id, $end_id, null);
                            ...
                            $this->quadId += 1;
                            $id = $this->quadId;
                            $this->quads[$this->quadId] = new Q_IR($this->quadId, 'Truncation', null, null, null);
                        }
                    }
                } else {
                    global $SinkAll;
                    //方法表里有的函数，或者sink里的自定义函数才生成逻辑，不然不管他  这里有点问题 因为可能是transfer
                    if (array_key_exists(md5($expr->name), $this->funcs)) {
                        ...
                        for ($i = 0; $i < $param_count; $i++) {
                            ...
                            $this->quads[$this->quadId] = new Q_IR($this->quadId, "UserFunc_Call_Param", null, null, $expr->args[$i]);
                        }
                        $this->quadId += 1;
                        $id = $this->quadId;
                        //{"Expr_FuncCall",方法名,null，null}
                        $this->quads[$this->quadId] = new Q_IR($this->quadId, $expr->getType(), $expr->name, null, null);
                        $this->quads[$id]->dest = $this->quadId + 1;
                        $end_id = $this->quadId;
                        $this->quadId += 1;
                        //arg表示从start_id到end_id的范围，即函数的开始到结束范围
                        $this->quads[$this->quadId] = new Q_IR($this->quadId, "Expr_FuncCall_Finish", $start_id, $end_id, null);
                    } elseif (array_key_exists($expr->name, $SinkAll)) {
                        //获取自定义函数的参数总数
                        $param_count = count($expr->args);
                        //对自定义函数的每一个参数生成quad
                        $start_id = $this->quadId + 1;
